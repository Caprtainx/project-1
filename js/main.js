...
function setMines() {
  ...
  while (minesLeft > 0) {
    ...
    if (!locationOfMines.includes(id)) {
      // prevents more then one bomb being randomized to the same square
...
function checkMine(r, c) {
  ...
  if (r < 0 || r >= rows || c < 0 || c >= columns) {
    // check if the r and c are within the board
    return;
  }
  ...
  if (minesFound > 0) {
    // this shows the number on the row or column around the click of how many bombs there are
    board[r][c].innerText = minesFound;
    board[r][c].classList.add("number" + minesFound.toString()); // adds the css for the number
  } else {
    // flood animation using recursion
    checkMine(r - 1, c - 1);
    checkMine(r - 1, c);
    checkMine(r - 1, c + 1);
...
function checkSquare(r, c) {
  if (r < 0 || r >= rows || c < 0 || c >= columns) {
    // checks if the r and c are within the board
    return 0;
  }
  if (locationOfMines.includes(r.toString() + "-" + c.toString())) {
    // if clicked on within the board, the array for r and c is created with the string of "r#-c#"
    return 1;
  }
  return 0;
}
